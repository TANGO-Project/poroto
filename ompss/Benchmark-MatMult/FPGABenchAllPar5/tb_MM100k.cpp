...
//{ 
    ...
    /*In this version of programme, the flag is at one for the turn first of loop*/    
    flag=1;
	...
	#pragma omp target device (smp) copy_deps
        #pragma omp task  in(A_1,A_2,B_1,B_2,time_top, time_end)  out(c_expected_tab,C_2,iter_fpga,iter_cpu,time_top, time_end,time_cpu,time_fpga,flag)
	{
	     #pragma omp task  in(A_1,A_2,B_1,B_2,time_top, time_end)  out(c_expected_tab,C_2,iter_fpga,iter_cpu,time_top, time_end,time_cpu,time_fpga,flag)
	    {	         			
		if(flag)
	    ...
	    /*this task exists just to count of the iterations on cpu and to suppress the dependency*/
