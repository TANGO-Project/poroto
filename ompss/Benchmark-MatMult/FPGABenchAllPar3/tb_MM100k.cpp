...
const char* name_file= "../__ExecStats/benchCPU_FPGA_3Par.csv";
int main() {
	...
	position = i;
	{
            	...
            	if(!flag)
	   	{
                	#pragma omp target device (smp)
                        #pragma omp task in(A_1,B_1) inout(c_expected_tab,iter_cpu,time_top,time_end,time_cpu)
                        {
		...
		if(flag)
        	{
	    		flag=0;
            		gettimeofday(&time_top[position],NULL);
			#pragma omp target device (smp)
			#pragma omp task in(A_2,B_2) inout(C_2,iter_fpga)
	    		{
