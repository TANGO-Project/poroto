...
const char* name_file= "../__ExecStats/benchmarkingCPU_SEQ.csv";
int main() {
 ...
 //*****************************************************************************
    ...
    fputs("Test of Matrice multiplication only on CPU\n",file);
    //*****************************************************************************
...
for(c=0 ; c < NB_LOOP ; c++){ 
	...
	{
	    gettimeofday(&time_top[i],NULL);
	    c_expected_tab[i] = calcMatrix( A_1,  B_1, height_A, width_B ,width_A_height_B);
	    gettimeofday(&time_end[i],NULL);
	    time_cpu+= ((time_end[i].tv_sec - time_top[i].tv_sec) * 1000000 + time_end[i].tv_usec) - time_top[i].tv_usec;
	}
    ...
    fprintf(file, "temps total CPU = %f\n",time_cpu/1000000);
